...
// 🔥 REBIRTH XP BOOST (1.5x compounding)
...
// ---------------- ROLL ----------------
function roll(luck) {
  const check = (c,n,d)=> Math.random() < c*luck ? {name:n,display:d} : null;
...
// ---------------- AUTOROLL ----------------
function getAutoDelay(rebirths){
  if(rebirths < 1) return null;
  let base = 10000;
  let reduction = Math.min(Math.max(rebirths - 2,0),5);
  return base - (reduction * 1000);
}

function startAutoRoll(user, msg){
  const delay = getAutoDelay(user.rebirths);
  if(!delay) return;

  if(autoRollers[msg.author.id]) return;

  autoRollers[msg.author.id] = setInterval(async () => {
    if(!msg.guild) return;
...
if(msg.content==="?roll"){
  ...
  const xpGain = Math.floor((points[r.name]||1) * getXpMultiplier(u.rebirths));
  u.xp += xpGain;
  ...
  const e = getRarityEffect(r.name);
...
if(msg.content==="?rebirth"){
  ...
  .setDescription(`Required Rolls: ${req}`)
  .addFields(
    {name:"Rebirth 1",value:"🔓 Autoroll unlocked (10s)"},
    {name:"Rebirth 2",value:"📈 XP multiplier 1.5x per rebirth"},
    {name:"Rebirth 3+",value:"⚡ Autoroll speed increases (-1s per rebirth up to rebirth 5)"}
  );
...
if(msg.content==="?rebirth confirm"){
  const req=Math.floor(1000*Math.pow(2.5,u.rebirths));
  if(u.rolls<req) return msg.reply("Not enough rolls");
